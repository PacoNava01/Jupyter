import os
import cv2
import numpy as np
from hailo_platform import (
    HEF, VDevice, HailoStreamInterface, InferVStreams,
    ConfigureParams, InputVStreamParams, OutputVStreamParams
)

COCO_CLASSES = [
    "person", "bicycle", "car", "motorcycle", "airplane", "bus", "train", "truck", "boat", "traffic light",
    "fire hydrant", "stop sign", "parking meter", "bench", "bird", "cat", "dog", "horse", "sheep", "cow",
    "elephant", "bear", "zebra", "giraffe", "backpack", "umbrella", "handbag", "tie", "suitcase", "frisbee",
    "skis", "snowboard", "sports ball", "kite", "baseball bat", "baseball glove", "skateboard", "surfboard",
    "tennis racket", "bottle", "wine glass", "cup", "fork", "knife", "spoon", "bowl", "banana", "apple",
    "sandwich", "orange", "broccoli", "carrot", "hot dog", "pizza", "donut", "cake", "chair", "couch",
    "potted plant", "bed", "dining table", "toilet", "tv", "laptop", "mouse", "remote", "keyboard", "cell phone",
    "microwave", "oven", "toaster", "sink", "refrigerator", "book", "clock", "vase", "scissors", "teddy bear",
    "hair drier", "toothbrush"
]

BASE_DIR = "/home/pacon/Jupyter/Tesis-Proyecto/Yolo_dir"
HEF_PATH = os.path.join(BASE_DIR, "yolov8n_seg.hef")
IMAGE_PATH = os.path.join(BASE_DIR, "test_game.jpg")
OUTPUT_PATH = os.path.join(BASE_DIR, "test_segmented.jpg")

CONF_THRESHOLD = 0.40
MASK_THRESHOLD = 0.50

def sigmoid(x):
    return 1.0 / (1.0 + np.exp(-x))

def run_segmentation():
    # 1. Cargar imagen de prueba
    img_original = cv2.imread(IMAGE_PATH)
    if img_original is None:
        raise FileNotFoundError(f"No se encontró la imagen en: {IMAGE_PATH}")
    
    h_orig, w_orig = int(img_original.shape[0]), int(img_original.shape[1])
    img_rgb = cv2.cvtColor(img_original, cv2.COLOR_BGR2RGB)

    # 2. Cargar modelo HEF
    hef = HEF(HEF_PATH)
    input_info = hef.get_input_vstream_infos()[0]
    input_h, input_w = input_info.shape[0], input_info.shape[1]
    input_name = input_info.name

    # Preprocesamiento NPU
    resized_img = cv2.resize(img_rgb, (input_w, input_h))
    input_data = {input_name: np.expand_dims(resized_img, axis=0).astype(np.uint8)}

    # 3. Configurar Hardware Hailo-8L
    params = VDevice.create_params()
    with VDevice(params) as target:
        configure_params = ConfigureParams.create_from_hef(hef, interface=HailoStreamInterface.PCIe)
        network_group = target.configure(hef, configure_params)[0]
        network_group_params = network_group.create_params()

        input_vstream_params = InputVStreamParams.make(network_group)
        output_vstream_params = OutputVStreamParams.make(network_group)

        with network_group.activate(network_group_params):
            with InferVStreams(network_group, input_vstream_params, output_vstream_params) as infer_pipeline:
                print("Ejecutando inferencia de segmentación en Hailo-8L...")
                raw_results = infer_pipeline.infer(input_data)
                print("Inferencia completada.")

    # 4. Separación de tensores: Prototipos vs Detecciones NMS
    proto_mask = None
    detections = None

    for key, value in raw_results.items():
        tensor = np.array(value)
        # Identificar mapa convolucional de prototipos (32 canales)
        if 32 in tensor.shape and len(tensor.shape) >= 3:
            proto_mask = tensor[0] if tensor.ndim == 4 else tensor
        else:
            detections = value[0] if isinstance(value, list) else tensor[0]

    output_img = img_original.copy()
    overlay = img_original.copy()

    # 5. Decodificación y Renderizado
    if detections is not None:
        det_array = np.array(detections)

        # Iterar sobre las clases
        for class_id in range(det_array.shape[0]):
            boxes = det_array[class_id]
            if len(boxes) == 0:
                continue

            for det in boxes:
                if len(det) >= 5:
                    ymin, xmin, ymax, xmax, score = [float(v) for v in det[:5]]

                    if score >= CONF_THRESHOLD:
                        # Normalizar coordenadas de forma segura al rango [0.0, 1.0]
                        if xmax > 1.0 or ymax > 1.0:
                            n_xmin = xmin / float(input_w)
                            n_ymin = ymin / float(input_h)
                            n_xmax = xmax / float(input_w)
                            n_ymax = ymax / float(input_h)
                        else:
                            n_xmin, n_ymin, n_xmax, n_ymax = xmin, ymin, xmax, ymax

                        # Desnormalizar a píxeles de la imagen original
                        x1 = max(0, min(int(n_xmin * w_orig), w_orig - 1))
                        y1 = max(0, min(int(n_ymin * h_orig), h_orig - 1))
                        x2 = max(0, min(int(n_xmax * w_orig), w_orig))
                        y2 = max(0, min(int(n_ymax * h_orig), h_orig))

                        box_w = x2 - x1
                        box_h = y2 - y1

                        # Descartar detecciones degeneradas o con área nula
                        if box_w < 5 or box_h < 5:
                            continue

                        # Asignar color según ID de clase
                        np.random.seed(int(class_id) + 10)
                        color = [int(c) for c in np.random.randint(60, 255, size=3)]

                        # Dibujar Bounding Box
                        cv2.rectangle(output_img, (x1, y1), (x2, y2), color, 2)

                        # Procesamiento de la Máscara
                        if proto_mask is not None:
                            mask_h, mask_w = proto_mask.shape[0], proto_mask.shape[1]
                            
                            # Coordenadas recortadas al tamaño del mapa proto (160x160)
                            mx1 = max(0, min(int(n_xmin * mask_w), mask_w - 1))
                            my1 = max(0, min(int(n_ymin * mask_h), mask_h - 1))
                            mx2 = max(0, min(int(n_xmax * mask_w), mask_w))
                            my2 = max(0, min(int(n_ymax * mask_h), mask_h))

                            if (mx2 - mx1) > 0 and (my2 - my1) > 0:
                                sub_proto = proto_mask[my1:my2, mx1:mx2, :]
                                
                                # Si la detección incluye los 32 coeficientes de máscara (len >= 37)
                                if len(det) >= 37:
                                    mask_coeffs = np.array(det[5:37], dtype=np.float32)
                                    mask_crop = np.tensordot(sub_proto, mask_coeffs, axes=([2], [0]))
                                else:
                                    mask_crop = np.max(sub_proto, axis=-1)

                                mask_binary = (sigmoid(mask_crop) > MASK_THRESHOLD).astype(np.uint8)
                                mask_resized = cv2.resize(mask_binary, (box_w, box_h), interpolation=cv2.INTER_NEAREST)

                                # Colorear la silueta sobre el overlay
                                roi = overlay[y1:y2, x1:x2]
                                roi[mask_resized == 1] = color

                        # Etiqueta
                        label = COCO_CLASSES[class_id] if class_id < len(COCO_CLASSES) else f"ID:{class_id}"
                        caption = f"{label} {score:.2f}"
                        cv2.putText(output_img, caption, (x1, max(y1 - 6, 18)),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2)

        # Fusión alfa para transparencias limpias
        alpha = 0.40
        cv2.addWeighted(overlay, alpha, output_img, 1.0 - alpha, 0, output_img)

    # 6. Guardar resultado final
    cv2.imwrite(OUTPUT_PATH, output_img)
    print(f"Resultado procesado guardado correctamente en: {OUTPUT_PATH}")

if __name__ == "__main__":
    run_segmentation()